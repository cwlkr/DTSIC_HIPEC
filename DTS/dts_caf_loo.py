...
# CAF only, original cluster names from deconv
features_selected = [i for i in data_org.columns if 'Fibroblast' in i]
...
roman_numerals = [0, 'I', 'II', 'III', 'IV', 'V', 'VI', 'VII', 'VIII']
mapping = {i:'CAF_' + str(roman_numerals[int(re.search("\\d", i).group())]) +':T' for i in features_selected if ':treatment' in i}
...
f1.savefig(os.path.join(outdir, f'KM_DTS_CAF_HIGH.png'), dpi=400)
f2.savefig(os.path.join(outdir, f'KM_DTS_CAF_LOW.png'), dpi=400)
...
mapping = {i:'CAF_' + str(roman_numerals[int(re.search("\\d", i).group())]) for i in features_selected if not ('treatment' in i)}
...
legend = ax.legend()
legend.texts[0].set_text(r"DTS$\mathregular{^{CAF}}$-LOW")
legend.texts[1].set_text(r"DTS$\mathregular{^{CAF}}$-HIGH")
plt.tight_layout()
fig.savefig(f'{outdir}/abundance_box_ordered_CAF.png', dpi=400)
plt.show()
...
plt.tight_layout()
fig.savefig(f'{outdir}/abundance_violin_CAF.png', dpi=400)
...
ax.set_ylim(-3,3)
plt.show()
# fig.savefig(f'{outdir}/scatterplot_time_DTS_CAF.png', dpi=400)
