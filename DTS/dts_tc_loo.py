...
# TC only, original cluster names from deconv
features_selected = [i for i in data_org.columns if 'Tumor_cells' in i]
...
mapping = {i:'TC_' + str(roman_numerals[int(re.search("\\d", i).group())]) +':T' for i in features_selected if ':treatment' in i}
...
f1.savefig(os.path.join(outdir, f'KM_DTS_TC_HIGH.png'), dpi=400)
f2.savefig(os.path.join(outdir, f'KM_DTS_TC_LOW.png'), dpi=400)
...
mapping = {i:'TC_' + str(roman_numerals[int(re.search("\\d", i).group())]) for i in features_selected if not ('treatment' in i)}
...
legend = ax.legend()
legend.texts[0].set_text(r"DTS$\mathregular{^{TC}}$-LOW")
legend.texts[1].set_text(r"DTS$\mathregular{^{TC}}$-HIGH")
plt.tight_layout()
fig.savefig(f'{outdir}/abundance_box_ordered_TC.png', dpi=400)
plt.show()
...
plt.tight_layout()
fig.savefig(f'{outdir}/abundance_violin_TC.png', dpi=400)
...
ax.set_ylim(-3,3)
plt.show()
# fig.savefig(f'{outdir}/scatterplot_time_DTS_TC.png', dpi=400)
